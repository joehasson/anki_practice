...
type _ t = 
  | Lf: leaf t 
  | Br: int * color * _ t * _ t -> node t

let rebalance: type a. a t -> a t = function
...
let rec insert_aux: type a. a t -> int -> node t = fun t x -> match t with
| Br (y,c,t1,t2) ->
    if x=y then t
...
let fold_inorder: type a. ('acc -> a t -> 'acc) -> 'acc -> a t -> 'acc = fun f init t ->
...
let size = fold_inorder (fun _ acc -> acc + 1) 0
let from_list = List.fold_left insert Lf
let empty = Lf
